...
checkpoint_csv <- file.path(out_dir, sprintf("WPP_indicator49_checkpoint_%d_%d.csv", start_year, end_year))
# Write header once
writeLines("LocationId,ISO3,ISO2,Location,Year,Total_Population", checkpoint_csv)
...
for (i in seq_len(nrow(countries))) {
  ...
  if (nrow(raw) == 0) {
    results[[i]] <- tibble(
      ...
      Location = character(), Year = integer(), Total_Population = numeric()
    )
    next
  }

  pop <- filter_common(raw) %>%
    group_by(Year) %>%
    summarise(Total_Population = first(Value), .groups = "drop")
