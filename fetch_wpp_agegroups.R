...
# Age groups: 0-14, 15-24, 25-64, 65+
...
pkgs <- c("httr", "jsonlite", "dplyr", "stringr", "readr", "writexl", "tidyr")
...
ag <- get_indicator(71) %>%
  filter_common(both_sexes = TRUE, extra_cols = list(
    age_label = c("age_label", "agelabel", "age_group", "agegroup",
                  "age_group_label", "agegrouplabel")
  )) %>%
  mutate(AgeGroup = vapply(age_label, normalize_age_group, character(1))) %>%
  filter(!is.na(AgeGroup))

if (nrow(ag) == 0) {
  stop("Indicator 71: 0 rows after age-group mapping.\n",
       "Inspect available labels with: unique(raw$age_label)")
}
...
# ---- Export ----
out_csv  <- file.path(out_dir, sprintf("WPP_world_agegroups_%d_%d.csv",  start_year, end_year))
out_xlsx <- file.path(out_dir, sprintf("WPP_world_agegroups_%d_%d.xlsx", start_year, end_year))

write_excel_csv(wide, out_csv)
write_xlsx(wide, out_xlsx)

message("Saved: ", out_csv, "\n       ", out_xlsx)
message("Rows: ", nrow(wide), " (expected ", end_year - start_year + 1, ")")
