#!/usr/bin/env python3
"""
Create a professional PowerPoint (v2) for East Asia & Pacific HCI+ data.
v2: ISO3 labels on bubbles, fixed region name cutoff, 3-letter region
codes, world orange markers, 6 employment indicators, scatter markers only.
"""
...
hseries.points[6].format.fill.fore_color.rgb = ORANGE
...
for i, (label, value, unit, num_val, max_val, color, world_lbl, best_lbl, world_num) in enumerate(metrics):
    ...
    # World avg orange dash marker on the bar
    world_x = bar_left_pos + int(bar_width_pos * (world_num / max_val))
    shape_rect(s5, world_x, Inches(3.75), Pt(2), Inches(0.32), ORANGE)
    text_box(s5, world_x - Inches(0.25), Inches(4.05), Inches(0.6), Inches(0.15),
             "World", size=6, color=ORANGE, align=PP_ALIGN.CENTER)
...
eseries.points[6].format.fill.fore_color.rgb = ORANGE
...
emp_row1 = [
    ...
    ("Emp. Rate Youth\n(15\u201324)", "53.0%", "EAP average", 53.0, 100, RGBColor(0xD0, 0x6D, 0x12),
...
emp_row2 = [
    ...
    ("Emp. Rate Working Age\n(25\u201365)", "72.5%", "EAP average", 72.5, 100, RGBColor(0x00, 0x70, 0x80),
...
text_box(s6, Inches(0.5), Inches(1.7), Inches(3), Inches(0.25),
         "YOUTH (15\u201324)", size=10, color=ORANGE, bold=True)
...
for i, (label, value, unit, num_val, max_val, color, world_lbl, best_lbl, world_num) in enumerate(emp_row1):
    ...
    # World avg orange dash
    w_x = bar_l + int(bar_w * (world_num / max_val))
    shape_rect(s6, w_x, y_top + Inches(0.88), Pt(2), Inches(0.25), ORANGE)
    text_box(s6, w_x - Inches(0.2), y_top + Inches(1.12), Inches(0.5), Inches(0.12),
             "World", size=5, color=ORANGE, align=PP_ALIGN.CENTER)
...
text_box(s6, Inches(0.5), Inches(3.85), Inches(3), Inches(0.25),
         "WORKING AGE (25\u201365)", size=10, color=TEAL, bold=True)
...
# Bottom — Regional employment comparison (narrower to fit with insight)
bottom_left = shape_rounded(s6, Inches(0.5), Inches(5.95), Inches(7.5), Inches(0.85), WHITE)

text_box(s6, Inches(0.8), Inches(6.0), Inches(5), Inches(0.2),
         "EMPLOYMENT: REGIONAL COMPARISON", size=9, color=MEDIUM_GRAY, bold=True)
...
emcf = s6.shapes.add_chart(
    XL_CHART_TYPE.BAR_CLUSTERED, Inches(0.5), Inches(6.15), Inches(7.0), Inches(0.65),
    emp_reg_data
)
emc = emcf.chart
emc.has_legend = False
emp_plot = emc.plots[0]
emp_plot.gap_width = 40
...
emseries.points[3].format.fill.fore_color.rgb = ORANGE

emseries.has_data_labels = True
emseries.data_labels.font.size = Pt(7)
emseries.data_labels.font.bold = True
emseries.data_labels.number_format = '0.0'

emc.category_axis.tick_labels.font.size = Pt(7)
emc.value_axis.visible = False
emc.value_axis.has_major_gridlines = False

# Insight (compact)
insight6 = shape_rounded(s6, Inches(7.8), Inches(5.95), Inches(5.0), Inches(0.85), WHITE)
shape_rect(s6, Inches(7.8), Inches(5.95), Pt(4), Inches(0.85), ORANGE)
text_box(s6, Inches(8.05), Inches(5.98), Inches(4.6), Inches(0.2),
         "KEY INSIGHT", size=9, color=ORANGE, bold=True)
text_box(s6, Inches(8.05), Inches(6.18), Inches(4.6), Inches(0.55),
...
# ── Save ──────────────────────────────────────────────────────────────────────
output_path = "/home/user/arcidiaconom.github.io/EAP_HCI_Plus_2025_v2.pptx"
